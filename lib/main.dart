...
class _HomeScreenState extends State<HomeScreen>{
  ...
  @override Widget build(BuildContext context)=>Scaffold(
    ...
    body:RefreshIndicator(onRefresh:_load,child:ListView(padding:const EdgeInsets.fromLTRB(16,8,16,100),children:[
      ...
      const SizedBox(height:18),if(loading)const Center(child:Padding(padding:EdgeInsets.all(30),child:CircularProgressIndicator())) else if(listings.isEmpty)const Card(child:Padding(padding:EdgeInsets.all(28),child:Column(children:[Icon(Icons.search_off,size:46),SizedBox(height:8),Text('آگهی‌ای پیدا نشد.'),Text('جستجو یا دسته‌بندی را تغییر دهید.')]))) else ...listings.map((p)=>ListingCard(product:p,onTap:()=>_details(p)))
    ])));
...
class _MonetizationScreenState extends State<MonetizationScreen>{
  ...
  void _msg(Object e)=>ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e.toString().replaceFirst('Exception: ',''))));
  Future<void> _buy(Map<String,dynamic> pkg)async{if(products.isEmpty){_msg(Exception('اول یک آگهی ثبت کنید.'));return;}final id=await showDialog<String>(context:context,builder:(_)=>SimpleDialog(title:const Text('انتخاب آگهی'),children:products.map((p)=>SimpleDialogOption(onPressed:()=>Navigator.pop(context,p['id'].toString()),child:Text(p['title']??''))).toList()));if(id==null)return;try{await ApiService.buyPromotion(listingId:id,packageId:pkg['id'].toString());if(mounted){_msg(Exception('ارتقا با موفقیت فعال شد.'));_load();}}catch(e){_msg(e);}}
  Future<void> _sub(String plan,String title,int price)async{final ok=await showDialog<bool>(context:context,builder:(_)=>AlertDialog(title:Text(title),content:Text('هزینه $price افغانی از کیف پول کسر می‌شود.'),actions:[TextButton(onPressed:()=>Navigator.pop(context,false),child:const Text('لغو')),FilledButton(onPressed:()=>Navigator.pop(context,true),child:const Text('تأیید'))]));if(ok!=true)return;try{await ApiService.buySubscription(plan);_msg(Exception('اشتراک فعال شد.'));_load();}catch(e){_msg(e);}}
  @override Widget build(BuildContext context){if(loading)return const Scaffold(body:Center(child:CircularProgressIndicator()));final bal=wallet['wallet'] is Map?(wallet['wallet']['balance_afn']??0):0;return Scaffold(appBar:AppBar(title:const Text('درآمد و تبلیغات')),body:RefreshIndicator(onRefresh:_load,child:ListView(padding:const EdgeInsets.all(16),children:[Card(child:ListTile(leading:const Icon(Icons.account_balance_wallet),title:const Text('موجودی کیف پول'),subtitle:Text('برای ارتقای آگهی و اشتراک استفاده می‌شود.'),trailing:Text('${bal.toString()} AFN',style:const TextStyle(fontSize:18,fontWeight:FontWeight.bold)))),const SizedBox(height:16),const Text('ارتقای آگهی',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:8),...packages.map((p)=>Card(child:ListTile(leading:Icon((p['pin_days']??0)>0?Icons.push_pin:Icons.star),title:Text(p['title']??''),subtitle:Text(p['description']??''),trailing:Text('${p['price_afn']} AFN'),onTap:()=>_buy(p)))),const SizedBox(height:16),const Text('فروشگاه حرفه‌ای',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:8),Card(child:ListTile(title:const Text('Basic'),subtitle:const Text('برای فروشنده‌های فعال'),trailing:const Text('500 AFN / ماه'),onTap:()=>_sub('basic','اشتراک Basic',500))),Card(child:ListTile(title:const Text('Pro'),subtitle:const Text('نمایش حرفه‌ای‌تر و امکانات بیشتر'),trailing:const Text('900 AFN / ماه'),onTap:()=>_sub('pro','اشتراک Pro',900))),Card(child:ListTile(title:const Text('Business'),subtitle:const Text('مناسب کسب‌وکارها'),trailing:const Text('1500 AFN / ماه'),onTap:()=>_sub('business','اشتراک Business',1500))),const SizedBox(height:16),if(orders.isNotEmpty)const Text('سفارش‌های تبلیغاتی',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),...orders.map((o)=>ListTile(title:Text((o['promotion_packages'] is Map?o['promotion_packages']['title']:o['package_id']).toString()),subtitle:Text('${o['amount_afn']} AFN • ${o['status']}'),leading:const Icon(Icons.receipt_long))),if(subs.isNotEmpty)const Text('اشتراک‌های من',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),...subs.map((x)=>ListTile(title:Text('پلن ${(x['plan']??'').toString()}'),subtitle:Text('تا ${(x['ends_at']??'').toString()}'),leading:const Icon(Icons.storefront))) ])));}
}
