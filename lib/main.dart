...
class _MonetizationScreenState extends State<MonetizationScreen>{
  ...
  @override Widget build(BuildContext context){if(loading)return const Scaffold(body:Center(child:CircularProgressIndicator()));final bal=wallet['wallet'] is Map?(wallet['wallet']['balance_afn']??0):0;return Scaffold(appBar:AppBar(title:const Text('🚀 بازارک BOOST')),body:RefreshIndicator(onRefresh:_load,child:ListView(padding:const EdgeInsets.all(16),children:[Card(child:Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('🚀 بازارک BOOST',style:TextStyle(fontSize:24,fontWeight:FontWeight.w900)),const SizedBox(height:6),const Text('آگهی‌ات را بیشتر دیده‌شدن بده! ویژه، پین و BOOST باعث می‌شود آگهی تو برجسته‌تر دیده شود.',style:TextStyle(height:1.5)),const SizedBox(height:10),const Text('پرداخت بانکی است و فقط بعد از تأیید مدیریت فعال می‌شود.',style:TextStyle(fontWeight:FontWeight.bold))]))),const SizedBox(height:14),Card(child:ListTile(leading:const Icon(Icons.account_balance_wallet),title:const Text('موجودی کیف پول'),subtitle:const Text('برای استفاده‌های آینده'),trailing:Text('${bal.toString()} AFN',style:const TextStyle(fontSize:18,fontWeight:FontWeight.bold)))),const SizedBox(height:18),const Text('🔥 بسته‌های BOOST و تبلیغ آگهی',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900)),const SizedBox(height:8),...packages.map((p)=>Card(child:ListTile(leading:CircleAvatar(child:Icon((p['boost_level']??0)>0?Icons.rocket_launch:(p['pin_days']??0)>0?Icons.push_pin:Icons.star)),title:Text(p['title']??'',style:const TextStyle(fontWeight:FontWeight.bold)),subtitle:Text(p['description']??''),trailing:Text('${p['price_afn']} AFN'),onTap:()=>_buy(p)))),const SizedBox(height:20),const Text('🏪 فروشگاه حرفه‌ای',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:8),Card(child:ListTile(title:const Text('Basic'),subtitle:const Text('برای فروشنده‌های فعال'),trailing:const Text('500 AFN / ماه'),onTap:()=>_sub('basic','اشتراک Basic',500))),Card(child:ListTile(title:const Text('Pro'),subtitle:const Text('نمایش حرفه‌ای‌تر و امکانات بیشتر'),trailing:const Text('900 AFN / ماه'),onTap:()=>_sub('pro','اشتراک Pro',900))),Card(child:ListTile(title:const Text('Business'),subtitle:const Text('مناسب کسب‌وکارها'),trailing:const Text('1500 AFN / ماه'),onTap:()=>_sub('business','اشتراک Business',1500))),const SizedBox(height:18),if(orders.isNotEmpty)const Text('📋 وضعیت سفارش‌های BOOST',style:TextStyle(fontSize:19,fontWeight:FontWeight.bold)),...orders.map((o)=>ListTile(title:Text((o['promotion_packages'] is Map?o['promotion_packages']['title']:o['package_id']).toString()),subtitle:Text('${o['amount_afn']} AFN • ${(o['status']=='pending')?'در انتظار تأیید پرداخت':o['status']}'),leading:const Icon(Icons.receipt_long))),if(subs.isNotEmpty)const Text('اشتراک‌های من',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),...subs.map((x)=>ListTile(title:Text('پلن ${(x['plan']??'').toString()}'),subtitle:Text('تا ${(x['ends_at']??'').toString()}'),leading:const Icon(Icons.storefront))) ])));}
}
