...
String provinceLabel(String province) => province == '__all__' ? 'کل افغانستان' : province;
...
class _HomeGateState extends State<HomeGate> {
  ...
  Future<void> _save(String value) async {
    final p=await SharedPreferences.getInstance();
    await p.setString('bazarek_province_filter', value);
    if(!mounted)return;
    setState(()=>province=value);
  }
  @override Widget build(BuildContext context){
    if(province==null) return ProvinceSelectionScreen(onSelected:_save);
...
class ProvinceSelectionScreen extends StatelessWidget {
  ...
  const ProvinceSelectionScreen({super.key,this.onSelected,this.current});
  Future<void> choose(BuildContext context,String value) async {
    if(onSelected!=null){await onSelected!(value);return;}
    if(context.mounted)Navigator.pop(context,value);
  }
  @override Widget build(BuildContext context)=>Scaffold(
    appBar:AppBar(title:const Text('انتخاب ولایت')),
    body:ListView(padding:const EdgeInsets.all(16),children:[
      const Card(child:Padding(padding:EdgeInsets.all(18),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Icon(Icons.location_on_outlined,size:40),SizedBox(height:10),
        Text('آگهی‌های کدام ولایت را می‌خواهید ببینید؟',style:TextStyle(fontSize:19,fontWeight:FontWeight.bold)),
        SizedBox(height:6),Text('ولایت انتخاب‌شده را بعداً می‌توانید تغییر دهید.'),
      ]))),
      const SizedBox(height:10),
      Card(child:ListTile(leading:const Icon(Icons.public),title:const Text('کل افغانستان'),subtitle:const Text('نمایش آگهی‌های تمام ولایات'),trailing:const Icon(Icons.chevron_left),onTap:()=>choose(context,'__all__'))),
      ...afghanistanProvinces.map((x)=>Card(child:ListTile(title:Text(x['fa']!),trailing:const Icon(Icons.chevron_left),onTap:()=>choose(context,x['fa']!)))),
    ]));
}
...
class _HomeScreenState extends State<HomeScreen>{
  ...
  @override void initState(){super.initState();_load();}
  Future<void> _load()async{if(mounted)setState(()=>loading=true);try{final x=await ApiService.getListings(q:search.text,category:category=='همه'?'':category,province:provinceFilter);if(mounted)setState(()=>listings=x);}catch(e){if(mounted)_msg(e);}finally{if(mounted)setState(()=>loading=false);}}
  ...
  @override Widget build(BuildContext context)=>Scaffold(
    appBar:AppBar(title:const Text('بازارک',style:TextStyle(fontWeight:FontWeight.bold)),actions:[IconButton(onPressed:() async { final r=await Navigator.push<String>(context,MaterialPageRoute(builder:(_)=>ProvinceSelectionScreen(current:provinceFilter))); if(r!=null&&mounted){final p=await SharedPreferences.getInstance();await p.setString('bazarek_province_filter',r);Navigator.pushReplacement(context,MaterialPageRoute(builder:(_)=>HomeScreen(provinceFilter:r)));}},tooltip:'تغییر ولایت',icon:const Icon(Icons.location_on_outlined)),IconButton(onPressed:_openMessages,tooltip:'پیام‌ها',icon:const Icon(Icons.chat_outlined)),IconButton(onPressed:_openAccount,tooltip:'حساب من',icon:const Icon(Icons.person_outline))]),
    ...
    body:RefreshIndicator(onRefresh:_load,child:ListView(padding:const EdgeInsets.fromLTRB(16,8,16,100),children:[
      Card(child:ListTile(leading:const Icon(Icons.location_on_outlined),title:Text(provinceLabel(provinceFilter),style:const TextStyle(fontWeight:FontWeight.bold)),subtitle:const Text('ولایت انتخاب‌شده'),trailing:const Icon(Icons.chevron_left),onTap:() async { final r=await Navigator.push<String>(context,MaterialPageRoute(builder:(_)=>ProvinceSelectionScreen(current:provinceFilter))); if(r!=null&&mounted){final p=await SharedPreferences.getInstance();await p.setString('bazarek_province_filter',r);Navigator.pushReplacement(context,MaterialPageRoute(builder:(_)=>HomeScreen(provinceFilter:r)));}})),
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            DropdownButtonFormField<String>(value:province.isEmpty?null:province,decoration:const InputDecoration(labelText:'ولایت آگهی',border:OutlineInputBorder()),items:afghanistanProvinces.map((x)=>DropdownMenuItem(value:x['fa'],child:Text(x['fa']!))).toList(),onChanged:(v)=>setState(()=>province=v??'')),
            const SizedBox(height: 10),
