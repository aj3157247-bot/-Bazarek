...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final PageController _professionalStoreController = PageController(viewportFraction: .90);
  Timer? _professionalStoreTimer;
  List<Map<String, dynamic>> _professionalStoresForHome = [];
  int _professionalStorePage = 0;
  String? loadError;
  ...
  void initState() {
    super.initState();
    _loadProducts();
    _professionalStoreTimer = Timer.periodic(const Duration(seconds: 6), (_) {
      if (!mounted || !_professionalStoreController.hasClients || _professionalStoresForHome.length < 2) return;
      final current = _professionalStoreController.page?.round() ?? 0;
      final next = (current + 1) % _professionalStoresForHome.length;
      _professionalStoreController.animateToPage(
        next,
  ...
  @override
  void dispose() {
    _professionalStoreTimer?.cancel();
    _professionalStoreController.dispose();
  ...
  Future<void> _showLanguagePicker() async {
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              title: const Text('فارسی / دری'),
              trailing: current == 'fa' ? const Icon(Icons.check_circle, color: Color(0xFF007185)) : null,
              onTap: () => Navigator.pop(sheetContext, 'fa'),
            ),
            ListTile(
              leading: const Text('🇦🇫', style: TextStyle(fontSize: 24)),
    ...
    BazarBuzurgApp.setLocale(context, Locale(selected));
  }
  ...
  Widget _buildWebStorefront({
    ...
    final horizontal = width >= 1100 ? 34.0 : (width >= 700 ? 22.0 : 10.0);
    final isMobile = width < 650;
    final isPs = Localizations.localeOf(context).languageCode == 'ps';

    // Professional storefronts: group the seller listings already returned by
    // the marketplace API. This keeps the homepage fast and lets a store open
    // its full seller catalog without introducing a second homepage data flow.
    final storeMap = <String, Map<String, dynamic>>{};
    final now = DateTime.now();
    bool isPaidActiveStore(Map<String, dynamic> item) {
      if (item['store_active'] != true) return false;
      final plan = item['store_plan']?.toString();
      if (plan != 'store_monthly' && plan != 'store_yearly') return false;
      final until = DateTime.tryParse(item['store_until']?.toString() ?? '');
      if (until == null || !until.isAfter(now)) return false;
      final starts = DateTime.tryParse(item['store_starts_at']?.toString() ?? '');
    ...
    for (final raw in products) {
      ...
      // Only paid, management-approved professional stores appear in the storefront.
      // Normal listings remain normal listings and never become a store automatically.
      if (vendorId.isEmpty || shopName.isEmpty || !isPaidActiveStore(item)) continue;
      final current = storeMap[vendorId];
      if (current == null) {
        storeMap[vendorId] = {
          'vendor_id': vendorId,
          'shop_name': shopName,
          'seller_name': item['seller_name']?.toString() ?? shopName,
        ...
        final price = _priceValue(item);
        if (price > 0 && (current['min_price'] as double) <= 0) {
          current['min_price'] = price;
        } else if (price > 0 && price < (current['min_price'] as double)) {
          current['min_price'] = price;
        }
      }
    }
    final professionalStores = storeMap.values.toList()..sort((a, b) => (b['count'] as int).compareTo(a['count'] as int));
    _professionalStoresForHome = professionalStores;
    ...
    String listingTime(dynamic raw) {
      final value = DateTime.tryParse(raw?.toString() ?? '');
      if (value == null) return '';
      final diff = DateTime.now().difference(value.toLocal());
      if (diff.inMinutes < 60) {
        return '${diff.inMinutes.clamp(1, 59)} دقیقه پیش';
      }
    ...
    Widget productCard(dynamic item) {
      ...
      final province = localizedProvince(
        context,
        item['province']?.toString() ?? '',
      );
      final location = (item['location_text'] ?? '').toString().trim();
      final boost = localizedBoostLabel(
        context,
        item['boost_label']?.toString() ?? '',
      );
      ...
      return InkWell(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              AspectRatio(
                aspectRatio: isMobile ? 1.12 : 1.28,
                child: Stack(
                  fit: StackFit.expand,
                  children: [
                    ClipRRect(
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (province.isNotEmpty || location.isNotEmpty)
                      Text(
                        [province, location]
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            // Top Amazon-like marketplace header, using Bazarek branding.
            Container(
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  children: [
                    ...
                    // Professional storefront — paid/approved stores only.
                    // Hidden completely when there is no active paid store.
                          ...
                          children: [
                            Row(children: [
                              Container(
                                width: 44, height: 44,
                                decoration: BoxDecoration(
                                  gradient: const LinearGradient(colors: [Color(0xFFFFD814), Color(0xFFFFB300)]),
                                  borderRadius: BorderRadius.circular(13),
                                ),
                                child: const Icon(Icons.storefront_rounded, color: Color(0xFF131921), size: 26),
                              ),
                              const SizedBox(width: 10),
                              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Text(isPs ? 'د بازارک مسلکي پلورنځي' : 'فروشگاه‌های حرفه‌ای بازارک', style: TextStyle(fontSize: isMobile ? 19 : 25, fontWeight: FontWeight.w900, color: const Color(0xFF131921))),
                                const SizedBox(height: 3),
                                Text(isPs ? 'محصولات په کټګوریو کې وګورئ او مستقیم خرید وکړئ.' : 'محصولات را دسته‌بندی‌شده ببینید و مستقیم از فروشگاه خرید کنید.', style: const TextStyle(fontSize: 11, color: Colors.black54, height: 1.35)),
                              ])),
                              if (professionalStores.length > 1) const Row(children: [Icon(Icons.swipe_rounded, size: 19, color: Color(0xFF007185)), SizedBox(width: 3), Text('بکشید', style: TextStyle(fontSize: 10, color: Color(0xFF007185), fontWeight: FontWeight.w900))]),
                            ]),
                            const SizedBox(height: 12),
                            SizedBox(
                              height: isMobile ? 286 : 315,
                              child: ListView.separated(
                                scrollDirection: Axis.horizontal,
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  final storeProducts = products.where((raw) => raw is Map && raw['vendor_id']?.toString() == vendorId && raw['store_active'] == true && (raw['store_plan'] == 'store_monthly' || raw['store_plan'] == 'store_yearly')).map((raw) => Map<String, dynamic>.from(raw as Map)).toList();
                                  final storeImage = _professionalFirstImage(storeProducts.isNotEmpty ? storeProducts.first : store);
                                  final previewProducts = storeProducts.take(3).toList();
                                  final cardWidth = isMobile ? 285.0 : 330.0;
                                  return InkWell(
                                    ...
                                    child: Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        borderRadius: BorderRadius.circular(14),
                                        boxShadow: const [BoxShadow(color: Color(0x24000000), blurRadius: 9, offset: Offset(0, 4))],
                                      ),
                                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        Row(children: [
                                          Container(width: 44, height: 44, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10)), child: storeImage.isNotEmpty ? ClipRRect(borderRadius: BorderRadius.circular(10), child: Image.network(_optimizedImageUrl(storeImage), fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.storefront_rounded))) : const Icon(Icons.storefront_rounded, color: Color(0xFF37475A))),
                                          const SizedBox(width: 8),
                                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                            Text(shopName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w900)),
                                            const SizedBox(height: 2),
                                            Text('${storeProducts.length} ${isPs ? 'محصول' : 'محصول'} • ${isPs ? 'پلورنځی فعال' : 'فروشگاه فعال'}', style: const TextStyle(color: Color(0xFFD5D9D9), fontSize: 9, fontWeight: FontWeight.w700)),
                                          ])),
                                          const Icon(Icons.verified_rounded, color: Color(0xFFFFD814), size: 21),
                                        ]),
                                        const SizedBox(height: 7),
                                        Text(isPs ? 'د پلورنځي ټول محصولات، بیې او کټګورۍ وګورئ.' : 'همه محصولات، قیمت‌ها و دسته‌بندی‌های این فروشگاه را ببینید.', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFFE7E9EA), fontSize: 10, height: 1.35)),
                                        const SizedBox(height: 8),
                                        Expanded(child: Row(children: [
                                          for (var i = 0; i < previewProducts.length; i++) ...[
                                            Expanded(child: Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), clipBehavior: Clip.antiAlias, child: Stack(children: [
                                              Positioned.fill(child: _professionalFirstImage(previewProducts[i]).isNotEmpty ? Image.network(_optimizedImageUrl(_professionalFirstImage(previewProducts[i])), fit: BoxFit.cover, errorBuilder: (_, __, ___) => const ColoredBox(color: Color(0xFFF3F3F3), child: Icon(Icons.image_outlined))) : const ColoredBox(color: Color(0xFFF3F3F3), child: Icon(Icons.image_outlined))),
                                              Positioned(left: 4, right: 4, bottom: 4, child: Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 3), decoration: BoxDecoration(color: Colors.black.withOpacity(.72), borderRadius: BorderRadius.circular(5)), child: Text(_displayListingPrice(context, previewProducts[i]), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900))))
                                            ]))),
                                            if (i != previewProducts.length - 1) const SizedBox(width: 6),
                                          ],
                                          if (previewProducts.isEmpty) const Expanded(child: Center(child: Icon(Icons.inventory_2_outlined, color: Colors.white54, size: 42))),
                                        ])),
                                        const SizedBox(height: 8),
                                        SizedBox(width: double.infinity, child: FilledButton.icon(
                                          ...
                                          label: Text(isPs ? 'پلورنځي ته لاړ شئ' : 'ورود به فروشگاه و خرید', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 11)),
                                        )),
                                      ]),
                                    ),
                                  );
                            ...
                            const SizedBox(height: 9),
                            Row(children: [
                              const Icon(Icons.lock_outline_rounded, size: 15, color: Color(0xFF007185)),
                              const SizedBox(width: 5),
                              Expanded(child: Text(isPs ? 'یوازې تایید شوي او تادیه شوي مسلکي پلورنځي دلته ښکاري.' : 'فقط فروشگاه‌های دارای پلن پولی و تأییدشده مدیریت در این بخش نمایش داده می‌شوند.', style: const TextStyle(fontSize: 9, color: Colors.black54, fontWeight: FontWeight.w700))),
                              Text('${professionalStores.length} فروشگاه', style: const TextStyle(fontSize: 9, color: Color(0xFF007185), fontWeight: FontWeight.w900)),
                            ]),
                          ],
                        ),
                      ),
                    ...
                    // Existing marketplace controls remain available.
                                  ...
                                  ? Padding(
                                      ...
                                      child: Center(
                                        child: Text(
                                          psText(
                                            context,
                                            'هنوز هیچ آگهی فعالی ثبت نشده است.',
                                            'تر اوسه کوم فعال اعلان نشته.',
                                          ),
                                          textAlign: TextAlign.center,
                                        ),
                                      ),
...
String _professionalCategoryId(dynamic item) {
  final parts = <String>[];
  for (final key in ['category_name', 'category_title', 'category', 'subcategory_name', 'subcategory_title', 'title']) {
    final value = item[key]?.toString().trim();
    if (value != null && value.isNotEmpty) parts.add(value);
  }
  final haystack = parts.join(' ').toLowerCase();
  if (RegExp(r'کفش|بوټ|shoe|sneaker|کتانی').hasMatch(haystack)) return 'shoes';
  if (RegExp(r'بلوز|پیراهن|کمیس|تی[ -]?شرت|shirt|t-shirt|blouse|کورتۍ').hasMatch(haystack)) return 'shirts';
  if (RegExp(r'پتلون|شلوار|pants|trouser|jeans|جین').hasMatch(haystack)) return 'pants';
  if (RegExp(r'موبایل|گوشی|مبایل|iphone|samsung|mobile|phone').hasMatch(haystack)) return 'phones';
  if (RegExp(r'لپ[ -]?تاپ|کمپیوتر|تلویزیون|الکترون|لپټاپ|computer|laptop|tv|electronics').hasMatch(haystack)) return 'electronics';
  if (RegExp(r'موتر|موټر|خودرو|ماشین|car|vehicle|toyota|کرولا').hasMatch(haystack)) return 'vehicles';
  if (RegExp(r'خانه|فرنیچر|مبل|میز|چوکی|کور|فرنیچر|furniture|home').hasMatch(haystack)) return 'home';
  if (RegExp(r'عطر|آرایشی|لوازم آرایش|کریم|perfume|beauty|cosmetic').hasMatch(haystack)) return 'beauty';
  return 'other';
}
...
String _professionalCategoryTitle(BuildContext context, String id) {
  ...
  const fa = {
    ...
    'beauty': 'آرایشی و بهداشتی', 'other': 'سایر محصولات',
  };
  const psMap = {
    'shoes': 'بوټان', 'shirts': 'کمیس او بلوز', 'pants': 'پتلون', 'phones': 'موبایل',
    'electronics': 'برېښنايي توکي', 'vehicles': 'وسایط', 'home': 'کور او فرنیچر',
    'beauty': 'ښکلا او روغتیا', 'other': 'نور محصولات',
  };
  return (ps ? psMap : fa)[id] ?? (ps ? 'نور محصولات' : 'سایر محصولات');
}

IconData _professionalCategoryIcon(String id) {
  switch (id) {
    case 'shoes': return Icons.shopping_bag_outlined;
    case 'shirts': return Icons.checkroom_outlined;
    case 'pants': return Icons.accessibility_new_outlined;
    case 'phones': return Icons.phone_android_outlined;
...
String _professionalProductCode(dynamic item) {
  final explicit = ['product_code', 'sku', 'listing_code', 'code']
      .map((key) => item[key]?.toString().trim() ?? '')
      .firstWhere((value) => value.isNotEmpty, orElse: () => '');
  if (explicit.isNotEmpty) return explicit;
  final raw = item['id']?.toString().trim() ?? '';
  if (raw.isEmpty) return 'BZ-UNKNOWN';
  final compact = raw.replaceAll(RegExp(r'[^A-Za-z0-9]'), '').toUpperCase();
  return 'BZ-${compact.length > 8 ? compact.substring(0, 8) : compact}';
}

String _professionalFirstImage(dynamic item) {
  try {
    final raw = item['image_url'];
...
Map<String, dynamic> _professionalSpecs(dynamic item) {
  try {
    final raw = item['product_specs'];
...
class ProfessionalStoreCatalogScreen extends StatefulWidget {
  ...
  const ProfessionalStoreCatalogScreen({super.key, required this.sellerId, required this.sellerName, this.initialCategory});

  @override
  State<ProfessionalStoreCatalogScreen> createState() => _ProfessionalStoreCatalogScreenState();
}

class _ProfessionalStoreCatalogScreenState extends State<ProfessionalStoreCatalogScreen> {
  bool loading = true;
  String? error;
  ...
  Map<String, Map<String, dynamic>> _categories() {
    ...
    for (final raw in listings) {
      if (raw is! Map) continue;
      final item = Map<String, dynamic>.from(raw);
      final id = _professionalCategoryId(item);
      final current = map[id];
      if (current == null) {
        map[id] = {'id': id, 'count': 1, 'image': _professionalFirstImage(item)};
      } else {
        current['count'] = (current['count'] as int) + 1;
      }
    }
  ...
  List<dynamic> get _visibleListings {
    if (selectedCategory.isEmpty) return listings;
    return listings.where((raw) => raw is Map && _professionalCategoryId(raw) == selectedCategory).toList();
  }

  Future<void> _copyCode(String code) async {
    await Clipboard.setData(ClipboardData(text: code));
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('کد کالا $code کپی شد.')));
  }
  ...
  Widget _categoryBar(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.category_outlined, color: Color(0xFF007185), size: 20),
          const SizedBox(width: 7),
          Expanded(child: Text(ps ? 'د پلورنځي کټګورۍ' : 'دسته‌بندی محصولات فروشگاه', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w900))),
          Text('${listings.length} ${ps ? 'محصول' : 'محصول'}', style: const TextStyle(fontSize: 11, color: Colors.black54, fontWeight: FontWeight.w700)),
        ]),
        const SizedBox(height: 10),
        SizedBox(
          height: 116,
            ...
            itemBuilder: (_, index) {
              if (index == 0) {
                return InkWell(
                  ...
                  child: Container(
                    width: 92,
                    decoration: BoxDecoration(color: selectedCategory.isEmpty ? const Color(0xFFE7F3F5) : const Color(0xFFF7F8F8), border: Border.all(color: selectedCategory.isEmpty ? const Color(0xFF007185) : const Color(0xFFE3E6E8)), borderRadius: BorderRadius.circular(8)),
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      const Icon(Icons.apps_rounded, size: 35, color: Color(0xFF007185)),
                      const SizedBox(height: 7),
                      Text(ps ? 'ټول' : 'همه محصولات', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w900), textAlign: TextAlign.center),
                    ]),
                  ),
                );
              }
              ...
              return InkWell(
                ...
                child: Container(
                  width: 92,
                  decoration: BoxDecoration(color: selected ? const Color(0xFFE7F3F5) : const Color(0xFFF7F8F8), border: Border.all(color: selected ? const Color(0xFF007185) : const Color(0xFFE3E6E8)), borderRadius: BorderRadius.circular(8)),
                  padding: const EdgeInsets.all(6),
                  child: Column(children: [
                    Expanded(
                      child: image.isNotEmpty
                          ? ClipRRect(borderRadius: BorderRadius.circular(5), child: Image.network(_optimizedImageUrl(image), width: double.infinity, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Icon(_professionalCategoryIcon(id), size: 30, color: const Color(0xFF4F659B))))
                          : Icon(_professionalCategoryIcon(id), size: 30, color: const Color(0xFF4F659B)),
                    ),
                    const SizedBox(height: 5),
                    Text(_professionalCategoryTitle(context, id), maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w900)),
                    Text('${cat['count']} ${ps ? 'توکي' : 'محصول'}', style: const TextStyle(fontSize: 8, color: Colors.black54)),
                  ]),
                ),
              );
            },
  ...
  Widget _productCard(BuildContext context, dynamic raw) {
          ...
          AspectRatio(
            ...
            child: ClipRRect(
              ...
              child: Stack(children: [
                Positioned.fill(child: image.isNotEmpty
                    ? Image.network(_optimizedImageUrl(image), fit: BoxFit.contain, loadingBuilder: _bazarekImageLoading, errorBuilder: (_, __, ___) => const ColoredBox(color: Color(0xFFF3F3F3), child: Icon(Icons.image_not_supported_outlined, size: 38, color: Colors.black38)))
                    : const ColoredBox(color: Color(0xFFF3F3F3), child: Icon(Icons.image_outlined, size: 38, color: Colors.black38))),
                Positioned(left: 8, bottom: 8, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: Colors.black.withOpacity(.72), borderRadius: BorderRadius.circular(6)), child: Text(price, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900)))),
                Positioned(right: 7, top: 7, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFFFD814), borderRadius: BorderRadius.circular(5)), child: Text(code, style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w900)))),
              ]),
            ),
          ),
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 5),
              Builder(builder: (_) {
                final specs = _professionalSpecs(item);
                final pairs = <String, String>{
                  'سایز': '${specs['sizes'] ?? specs['size'] ?? ''}'.trim(),
                  'رنگ': '${specs['colors'] ?? specs['color'] ?? ''}'.trim(),
                  'برند': '${specs['brand'] ?? ''}'.trim(),
                }..removeWhere((k, v) => v.isEmpty);
                return pairs.isEmpty ? const SizedBox.shrink() : Wrap(spacing: 4, runSpacing: 4, children: pairs.entries.take(3).map((e) => Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 3), decoration: BoxDecoration(color: const Color(0xFFF3F3F3), borderRadius: BorderRadius.circular(4)), child: Text('${e.key}: ${e.value}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w800)))).toList());
              }),
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      Container(
                        ...
                        child: Row(children: [
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            const SizedBox(height: 4),
                            Text(ps ? 'هر محصول د عکس، قیمت او ځانګړي کوډ سره.' : 'هر محصول با عکس، قیمت و کد اختصاصی نمایش داده می‌شود.', style: const TextStyle(color: Color(0xFFD5D9D9), fontSize: 11, height: 1.45)),
                            const SizedBox(height: 7),
                            Text(ps ? 'د اخیستلو پر وخت د محصول کوډ ووایاست.' : 'برای خرید، کافی است کد کالا را به فروشنده بگویید.', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800)),
                          ])),
                        ]),
                      ),
                      const SizedBox(height: 10),
                      ...
                      const SizedBox(height: 12),
                      Row(children: [
                        Expanded(child: Text(selectedCategory.isEmpty ? (ps ? 'ټول محصولات' : 'همه محصولات') : _professionalCategoryTitle(context, selectedCategory), style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900))),
                        Text('${_visibleListings.length}', style: const TextStyle(color: Colors.black54, fontWeight: FontWeight.w800)),
                      ]),
                      const SizedBox(height: 9),
                      if (_visibleListings.isEmpty)
                        Container(padding: const EdgeInsets.all(30), color: Colors.white, child: Text(ps ? 'په دې کټګورۍ کې محصول نشته.' : 'در این دسته محصولی پیدا نشد.', textAlign: TextAlign.center))
                      else
                        GridView.builder(
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          itemCount: _visibleListings.length,
                          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: columns, crossAxisSpacing: 10, mainAxisSpacing: 10, childAspectRatio: width < 600 ? .70 : .78),
                          itemBuilder: (_, i) => _productCard(context, _visibleListings[i]),
                        ),
                    ],
                  ),
                ),
...
class _BoostScreenState extends State<BoostScreen> {
  ...
  Future<void> _buyGlobal(String plan, int price, String title) async {
    ...
    try {
      await ApiService.createGlobalBoost(plan, ref);
      if (mounted) {
        final storePlan = plan.startsWith('store_');
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(Localizations.localeOf(context).languageCode == 'ps'
            ? (storePlan ? 'د مسلکي پلورنځي غوښتنه ثبت شوه؛ د تادیې له تایید وروسته فعاله کېږي.' : 'ستاسو غوښتنه ثبت شوه؛ د تایید وروسته ستاسو ټول فعال اعلانونه Boost کېږي.')
            : (storePlan ? 'درخواست فروشگاه حرفه‌ای ثبت شد؛ پس از تأیید پرداخت فعال می‌شود.' : 'درخواست ثبت شد؛ پس از تأیید پرداخت، روی همه آگهی‌های فعال شما اعمال می‌شود.'))));
        _load();
      }
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(friendlyNetworkError(context, e)))); }
  }
  ...
  Widget _globalBoostPlanCard(BuildContext context, bool ps, String plan, String title, String desc, int price) {
    ...
    return Card(child: ListTile(
      leading: Text(plan == 'boost_weekly' ? '⚡' : plan == 'boost_monthly' ? '👑' : plan == 'boost_yearly' ? '🏆' : plan == 'store_monthly' ? '🏪' : '👑', style: const TextStyle(fontSize: 30)),
    ...
    return Scaffold(
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : error != null ? OfflineErrorView(onRetry: _load, message: error) : ListView(
        ...
        children: [
          ...
          const SizedBox(height: 18),
          Text(ps ? '🏪 د مسلکي پلورنځي پلانونه' : '🏪 پلن‌های فروشگاه حرفه‌ای', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
          const SizedBox(height: 5),
          Text(ps ? 'پلورنځی په بازارک کې ځانګړی کړئ؛ ټول محصولات او بیې مو په یوه مسلکي پاڼه کې ښکاره کېږي.' : 'فروشگاه خود را در بازارک ویژه کنید؛ همه محصولات و قیمت‌ها در یک صفحه حرفه‌ای نمایش داده می‌شود.', style: const TextStyle(color: Colors.black54)),
          const SizedBox(height: 10),
          _globalBoostPlanCard(context, ps, 'store_monthly', ps ? '🏪 میاشتنی — مسلکي پلورنځی' : '🏪 ماهانه — فروشگاه حرفه‌ای', ps ? '۳۰ ورځې؛ صفحه فروشگاه و نمایش محصولات.' : '۳۰ روز؛ صفحه فروشگاه و نمایش حرفه‌ای محصولات.', 600),
          _globalBoostPlanCard(context, ps, 'store_yearly', ps ? '👑 کلنی — مسلکي پلورنځی' : '👑 سالانه — فروشگاه حرفه‌ای', ps ? '۳۶۵ ورځې؛ پلورنځی حرفه‌ای.' : '۳۶۵ روز؛ فروشگاه حرفه‌ای برای یک سال.', 6000),
          const SizedBox(height: 8),
...
class _AddProductSheetState extends State<AddProductSheet> {
  ...
  final condition = TextEditingController(text: 'نو');
  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      final payload = jsonEncode({
        ...
        'product_specs': jsonEncode({
          'brand': brand.text.trim(), 'model': model.text.trim(), 'colors': colors.text.trim(),
          'sizes': sizes.text.trim(), 'material': material.text.trim(), 'condition': condition.text.trim(),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(color: const Color(0xFFF7F8F8), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE3E6E8))),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(psText(context, 'مشخصات حرفه‌ای محصول', 'د محصول مسلکي معلومات'), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15)),
              const SizedBox(height: 5),
              Text(psText(context, 'برای کفش، لباس و محصولات مشابه سایز و رنگ را با کاما جدا کنید؛ مثلاً 40,41,42 یا سیاه, سفید.', 'د بوټانو، جامو او ورته توکو لپاره سایزونه او رنګونه په کامه سره ولیکئ.'), style: const TextStyle(fontSize: 10, color: Colors.black54, height: 1.35)),
              const SizedBox(height: 9),
              Row(children: [Expanded(child: TextField(controller: brand, decoration: InputDecoration(labelText: psText(context, 'برند', 'برانډ'), prefixIcon: const Icon(Icons.branding_watermark_outlined)))), const SizedBox(width: 8), Expanded(child: TextField(controller: model, decoration: InputDecoration(labelText: psText(context, 'مدل', 'ماډل'), prefixIcon: const Icon(Icons.tag_outlined))))]),
              const SizedBox(height: 8),
              Row(children: [Expanded(child: TextField(controller: sizes, decoration: InputDecoration(labelText: psText(context, 'سایزها', 'سایزونه'), hintText: '40, 41, 42'))), const SizedBox(width: 8), Expanded(child: TextField(controller: colors, decoration: InputDecoration(labelText: psText(context, 'رنگ‌ها', 'رنګونه'), hintText: 'سیاه, سفید')))]),
              const SizedBox(height: 8),
              Row(children: [Expanded(child: TextField(controller: material, decoration: InputDecoration(labelText: psText(context, 'جنس', 'جنس')))), const SizedBox(width: 8), Expanded(child: TextField(controller: condition, decoration: InputDecoration(labelText: psText(context, 'وضعیت', 'حالت'))))]),
            ]),
          ),
          const SizedBox(height: 12),
          TextField(
