...
class ApiService {
  ...
  static Future<Map<String,dynamic>> updateProfile({required String fullName, String shopName='', String phone='', String city='', String bio=''}) async { final r=await http.patch(Uri.parse('$baseUrl/me'),headers:_headers(auth:true),body:jsonEncode({'full_name':fullName.trim(),'shop_name':shopName.trim(),'phone':phone.trim(),'city':city.trim(),'bio':bio.trim()})); final d=_json(r); if(r.statusCode!=200)throw Exception(d['error']??'خطا در ذخیره پروفایل.'); return Map<String,dynamic>.from(d); }
  static Future<List<Map<String,dynamic>>> getSupportRequests() async { final r=await http.get(Uri.parse('$baseUrl/support/requests'),headers:_headers(auth:true)); final d=_json(r); if(r.statusCode!=200)throw Exception(d['error']??'خطا در دریافت درخواست‌ها.'); return List<Map<String,dynamic>>.from((d as List).map((e)=>Map<String,dynamic>.from(e))); }
  static Future<Map<String,dynamic>> createSupportRequest(String type,String title,String message) async { final r=await http.post(Uri.parse('$baseUrl/support/requests'),headers:_headers(auth:true),body:jsonEncode({'type':type,'title':title.trim(),'message':message.trim()})); final d=_json(r); if(r.statusCode!=201)throw Exception(d['error']??'خطا در ثبت درخواست.'); return Map<String,dynamic>.from(d); }
