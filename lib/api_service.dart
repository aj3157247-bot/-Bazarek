...
class ApiService {
  ...
  static Future<String> login(String phone,String password) async { final r=await http.post(Uri.parse('$baseUrl/auth/login'),headers:_headers(),body:jsonEncode({'phone':phone,'password':password})); final d=_json(r); if(r.statusCode!=200)throw Exception(d['error']??'ورود ناموفق بود.'); _token=d['token']?.toString(); _refreshToken=d['refresh_token']?.toString(); if(_token==null||_token!.isEmpty)throw Exception('ورود ناموفق بود.'); return _token!; }
  static Future<Map<String,dynamic>> register({required String phone,required String password,String fullName='',String shopName=''}) async { final r=await http.post(Uri.parse('$baseUrl/auth/register'),headers:_headers(),body:jsonEncode({'phone':phone,'password':password,'full_name':fullName,'shop_name':shopName})); final d=_json(r); if(r.statusCode!=201&&r.statusCode!=200)throw Exception(d['error']??'ثبت‌نام ناموفق بود.'); if(d['token']!=null)_token=d['token'].toString(); if(d['refresh_token']!=null)_refreshToken=d['refresh_token'].toString(); return Map<String,dynamic>.from(d); }
