...
class ApiService {
  ...
  static Future<void> adminSetSubscription(String token,String id,String status) async { final r=await http.patch(Uri.parse('$baseUrl/admin/subscriptions/$id'),headers:_headers(token:token),body:jsonEncode({'status':status})); final d=_json(r); if(r.statusCode!=200)throw Exception(d['error']??'خطا در تغییر وضعیت اشتراک.'); }
