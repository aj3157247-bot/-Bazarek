...
class ApiService {
  ...
  static Future<Map<String, dynamic>> getPaymentMethods() async {
    ...
    if (res.statusCode == 401 && await refreshSession()) {
      res = await http.get(Uri.parse('${ApiConfig.baseUrl}/payment-methods'), headers: headers).timeout(const Duration(seconds: 15));
    }
    final data = jsonDecode(res.body);
    if (res.statusCode != 200) throw Exception(data is Map ? (data['error'] ?? 'خطا در دریافت روش‌های پرداخت.') : 'خطا در دریافت روش‌های پرداخت.');
