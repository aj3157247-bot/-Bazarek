...
-- Idempotent migration; safe to run even if the table already exists.
...
CREATE INDEX IF NOT EXISTS saved_stores_user_idx
  ON public.saved_stores(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS saved_stores_seller_idx
  ON public.saved_stores(seller_id, created_at DESC);
...
CREATE POLICY saved_stores_self_write
